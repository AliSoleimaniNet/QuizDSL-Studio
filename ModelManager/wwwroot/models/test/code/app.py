...
if os.path.exists(CONFIG_FILE):
    ...
    key = 'b6fd9184-baf7-4939-832c-9bbf68a6d160'
...
@app.route('/quiz/SingleQuestionTest', methods=['GET', 'POST'])
def quiz_SingleQuestionTest():
    from datetime import datetime
    now = datetime.now()
    if request.method == 'POST':
        total_score = 0
        responses = []
        # بررسی سوال: SingleQuestionTest_q1
        ans = request.form.get('SingleQuestionTest_q1')
        is_correct = (ans == 'SingleQuestionTest_q1_o2')
        q_score = 20 if is_correct else 0
        total_score += q_score
        responses.append(UserResponse(question_name='SingleQuestionTest_q1', answer=ans, correct=is_correct, score_value=q_score))
        res = QuizResult(name=f"RES_{session.get('user_id')}_{int(now.timestamp())}", 
                         user_id=session.get('user_id'), quiz_name='SingleQuestionTest', 
    ...
    return render_template('quiz_SingleQuestionTest.html')
...
def my_results():
    ...
    enhanced_results = []
    # پردازش نتایج مربوط به آزمون: SingleQuestionTest
    for res in [r for r in db_results if r.quiz_name == 'SingleQuestionTest']:
        enhanced_results.append({
            'id': res.id,
            'quiz_id': res.name,
            'quiz_name': 'آزمون تک سوال',
...
def get_result_details(res_id):
    ...
    responses_list = []
    if res.quiz_name == 'SingleQuestionTest':
        if not is_admin:
            return jsonify({'error': 'نتایج غیرفعال است'}), 403
        user_resp = next((r for r in res.responses if r.question_name == 'SingleQuestionTest_q1'), None)
        raw_ans = user_resp.answer if user_resp else None
        display_ans = 'بدون پاسخ'
        if raw_ans:
            mapping = {'SingleQuestionTest_q1_o1': 'یک زبان اسکریپتی است.', 'SingleQuestionTest_q1_o2': 'یک زبان کامپایلری است.', 'SingleQuestionTest_q1_o3': 'فقط برای توسعه وب استفاده می‌شود.', 'SingleQuestionTest_q1_o4': 'یک زبان فقط برای بازی‌سازی است.', }
            display_ans = mapping.get(raw_ans, raw_ans)
        responses_list.append({
            'question_text': "کدام یک از موارد زیر در مورد C# صحیح است؟",
            'answer': display_ans,
...
def admin_dashboard():
    quiz_labels = {
        'SingleQuestionTest': 'آزمون تک سوال',
    }
...
def admin_results():
    ...
    quiz_map = { 'SingleQuestionTest': {'label': 'آزمون تک سوال', 'max': 20}, }
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not User.query.filter_by(username='ali_admin').first():
            db.session.add(User(username='ali_admin', password='admin123', full_name='علی', role='admin'))
        db.session.commit()
    app.run(debug=True)